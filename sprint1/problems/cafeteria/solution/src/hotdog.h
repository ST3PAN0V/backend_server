...
public:
    ...
    HotDog(int id, std::shared_ptr<Sausage> sausage, std::shared_ptr<Bread> bread)
        ...
        if (sausage_->GetCookDuration() < MIN_SAUSAGE_COOK_DURATION
            ...
            std::chrono::nanoseconds cookDuration = sausage_->GetCookDuration();
            std::chrono::milliseconds cookDurationInSeconds = std::chrono::duration_cast<std::chrono::milliseconds>(cookDuration);    
            std::cout << cookDurationInSeconds.count() << " milliseconds" << std::endl;
        ...
        if (bread_->GetBakingDuration() < MIN_BREAD_COOK_DURATION
            ...
            std::chrono::nanoseconds cookDuration = bread_->GetBakingDuration();
            std::chrono::milliseconds cookDurationInSeconds = std::chrono::duration_cast<std::chrono::milliseconds>(cookDuration);    
            std::cout << cookDurationInSeconds.count() << " milliseconds" << std::endl;
